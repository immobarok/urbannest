...
export class AuthController {
	...
	@Post("login")
	login(@Request() req: { user: Omit<User, "passwordHash"> }): AuthTokensEntity {
		return this.authService.login(req.user);
	}

	@Public()
	@Post("verify-email")
	async verifyEmail(@Body() dto: VerifyEmailDto): Promise<MessageResponseEntity> {
	...
	@Public()
	@Post("reset-password")
	async resetPassword(@Body() dto: ResetPasswordDto): Promise<MessageResponseEntity> {
